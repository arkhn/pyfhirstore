...
def element_search(params):
    ...
    for key, value in params.items():
        ...
        n = re.search(
            r"^(gt|lt|ge|le)([0-9].*)$",
            value)
        m = re.search(r"^(.*):(contains|exact|above|below|not|in|not-in|of-type)$", key)
        p = re.search(r"^(eq)([0-9].*)$",value)
        if m:
            ...
            if m.group(2) == "exact":
                parsed_params["query_string"]["query"] = value
                parsed_params["query_string"]["fields"] = [m.group(1)]

            # Term does not go in depth
            if m.group(2) == "not":
                parsed_params["bool"]["must_not"]["match"][m.group(1)] = f"{value}"
            # TODO: improve method for FHIR tokens
            if m.group(2) == "not-in":
                parsed_params["simple_query_string"]["query"] = f"-{value}"
